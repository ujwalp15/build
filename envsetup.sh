...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^px_") ; then
       PX_BUILD=$(echo -n $1 | sed -e 's/^px_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $PX_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       PX_BUILD=
    fi
    export PX_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    PX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/px/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the PX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch px_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var PX_VERSION)
        ZIPFILE=px-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.px.device=$PX_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $PX_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.px.device=$PX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/px/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
